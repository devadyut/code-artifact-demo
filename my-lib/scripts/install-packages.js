...
function configureRegistry(registryUrl = null) {
    console.log(`${colors.yellow}🔧 Configuring npm registry for @myorg packages...${colors.reset}`);
    ...
    if (!registryUrl) {
        try {
            ...
            if (fs.existsSync(configPath)) {
                ...
                console.log(`${colors.cyan}📋 Using CodeArtifact registry from config${colors.reset}`);
            } else {
                console.error(`${colors.red}❌ No CodeArtifact configuration found at ${configPath}${colors.reset}`);
                console.log(`${colors.yellow}💡 Please create codeartifact-config.json with AWS CodeArtifact settings${colors.reset}`);
                return false;
            }
        } catch (error) {
    ...
    if (!registryUrl) {
        console.error(`${colors.red}❌ No registry URL available${colors.reset}`);
        return false;
    }
...
function setupServicesOnly() {
    console.log(`${colors.bright}${colors.magenta}🚀 My App Libraries - Services Setup${colors.reset}\n`);
...
function setupUtilitiesOnly() {
    console.log(`${colors.bright}${colors.magenta}🚀 My Appf Libraries - Utilities Setup${colors.reset}\n`);
...
function setupDevelopmentEnvironment() {
    console.log(`${colors.bright}${colors.magenta}🚀 My App Libraries - Development Setup${colors.reset}\n`);
    ...
    try {
        ...
        if (packageJson.name !== '@myorg/libraries') {
            console.error(`${colors.red}❌ This doesn't appear to be the My App Libraries repository.${colors.reset}`);
            process.exit(1);
        }
    } catch (error) {
    ...
    console.log(`${colors.green}✅ Confirmed: My App Libraries repository${colors.reset}\n`);
...
function showUsage() {
    console.log(`${colors.bright}${colors.cyan}📖 My App Development Setup${colors.reset}`);
